...
export const updateAccountProfile = createServerFn({ method: "POST" }).validator(z.object({ name: z.string().trim().min(2).max(80), image: z.string().url().max(500).nullable(), neighborhood: z.string().trim().min(2).max(40) })).middleware([authMiddleware]).handler(async ({ data, context }) => { const sql = await getSql(); await sql`update "user" set "name" = ${data.name}, "image" = ${data.image}, "updatedAt" = now() where "id" = ${context.userId}`; await sql`insert into account_settings (user_id, default_neighborhood) values (${context.userId}, ${data.neighborhood}) on conflict (user_id) do update set default_neighborhood = excluded.default_neighborhood, updated_at = now()`; return { ok: true }; });
