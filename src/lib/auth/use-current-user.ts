...
export function useCurrentUserState(): CurrentUserState {
  ...
  // On server (SSR), we can't use the auth client - return pending state
 ...
 * 
 * During SSR or when not logged in, returns a "guest" user object to avoid
 * destructuring errors in components. Use `useCurrentUserState()` for auth logic.
 */
export function useCurrentUser(): AppUser {
  const { user, isPending } = useCurrentUserState();
  
  // During SSR or while loading, return a guest user to avoid destructuring errors
  if (isPending || !user) {
    return {
      id: "guest",
