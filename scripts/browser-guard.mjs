...
export function checkedOutputPath(target, allowedDirs, label = "screenshot") {
  ...
  // Also allow the current working directory (for Windows compatibility)
  const cwd = process.cwd();
  const cwdAllowed = abs.startsWith(cwd.endsWith(sep) ? cwd : cwd + sep);
  if (!allowed && !cwdAllowed) {
    fail(`${label} path must be under ${allowedDirs.join(" or ")} (or cwd), got ${abs}`);
  }
  return abs;
}
